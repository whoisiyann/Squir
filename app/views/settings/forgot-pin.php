...
<head>
    ...
    <meta name="description" content="Reset your Squir vault PIN.">
    <title>Squir - Forgot PIN</title>
...
<body>
    <main class="login-shell">
        <section class="auth-card" aria-labelledby="forgot-pin-title">
            <div class="auth-visual" aria-label="Squir mascot">
                <img src="./assets/images/squir-login-signin-left-image.png" alt="Squir mascot">
            </div>

            <div class="auth-form">
                <a class="auth-back" href="<?= $escape($backUrl ?? './settings?panel=reset-pin') ?>" aria-label="Back"><i class="ti ti-arrow-left" aria-hidden="true"></i></a>
                ...
                <h1 id="forgot-pin-title">Forgot your PIN?</h1>
                <p class="subtitle">No worries! Confirm the email on your account and we'll send you a code to reset your PIN.</p>

                <?php if (!empty($errors['form'])): ?>
                    <div class="alert-squir alert-danger-squir" role="alert"><?= $escape($errors['form']) ?></div>
                <?php endif; ?>

                <form method="post" action="./forgot-pin">
                ...
                <p class="auth-switch">Remember your PIN? <a href="<?= $escape($backUrl ?? './settings?panel=reset-pin') ?>">Go back</a></p>
            </div>
        </section>
    </main>
