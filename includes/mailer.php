...
function sendPinResetEmail(string $toEmail, string $toName, string $code, int $ttlMinutes): bool
{
    ...
    $subject = 'Your Squir PIN reset code';
    $safeName = htmlspecialchars($toName, ENT_QUOTES, 'UTF-8');

    $body = '<p>Hi ' . $safeName . ',</p>'
        . '<p>Use the code below to reset your Squir vault PIN:</p>'
        . '<p style="font-size:28px;font-weight:700;letter-spacing:6px;">' . htmlspecialchars($code, ENT_QUOTES, 'UTF-8') . '</p>'
        . '<p>This code expires in ' . $ttlMinutes . ' minutes. If you did not request this, you can safely ignore this email.</p>';

    $plainBody = "Hi {$toName},\n\nUse the code below to reset your Squir vault PIN:\n\n{$code}\n\nThis code expires in {$ttlMinutes} minutes. If you did not request this, you can safely ignore this email.";
    ...
    try {
        ...
        $mail->send();
        squirMailLog("SUCCESS sending PIN reset to {$toEmail} (auth user: {$mail->Username})");
        return true;
    } catch (PHPMailerException $e) {
        // Write mail errors to the PHP log.
        error_log('sendPinResetEmail failed: ' . $mail->ErrorInfo);
        squirMailLog("FAILED sending PIN reset to {$toEmail} (auth user: {$mail->Username}) — ErrorInfo: {$mail->ErrorInfo}");
        return false;
    }
}
